		...
		for(int i = 1; i < MAX_LISTEN_QUEUE; i++)
		{
			...
			if(temp_fds[i].revents & POLLIN) {
				...
				if(!strcmp(pm_data.cmd, PM_ENTER_CMD)) {
					...
					fprintf(stderr, SD_INFO "pm_cancel:%d prev_success_cnt %d cur_success_cnt %d\n",
						(pm_ops->pm_cancel != NULL) ? 1: 0, pm_hdl->prev_suspend_stat_success_cnt, cur_suspend_stat_success_cnt);
					...
					if ((pm_ops->pm_cancel != NULL) && (cur_suspend_stat_success_cnt == pm_hdl->prev_suspend_stat_success_cnt)) {
						fprintf(stderr, SD_INFO "DS rollback detected, invoking pm_cancel\n");
